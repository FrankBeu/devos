{
  ...
  ksp      = ''kubectl decribe pod                                             '';
  kspa     = ''kubectl decribe pod                --all-namespaces             '';

  ksd      = ''kubectl decribe deployment                                      '';
  ksda     = ''kubectl decribe deployment         --all-namespaces             '';

  kss      = ''kubectl decribe service                                         '';
  kssa     = ''kubectl decribe service            --all-namespaces             '';

  ksn      = ''kubectl decribe node                                            '';
  ksna     = ''kubectl decribe node               --all-namespaces             '';

  ksi      = ''kubectl decribe ingress                                         '';
  ksia     = ''kubectl decribe ingress            --all-namespaces             '';

  ksr      = ''kubectl decribe role                                            '';
  ksra     = ''kubectl decribe role               --all-namespaces             '';

  ksrb     = ''kubectl decribe rolebinding                                     '';
  ksrba    = ''kubectl decribe rolebinding        --all-namespaces             '';

  kscr     = ''kubectl decribe clusterrole                                     '';
  kscra    = ''kubectl decribe clusterrole        --all-namespaces             '';

  kscrb    = ''kubectl decribe clusterrolebinding                              '';
  kscrba   = ''kubectl decribe clusterrolebinding --all-namespaces             '';
